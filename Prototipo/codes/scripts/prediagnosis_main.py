from prediagnosis_classification_nn import image_classificator
...
def main():
    ...
    device = 'cuda'
    model = torch.load('./prediagnosis-model.pth', map_location=device)
    red = image_classificator(3).to('cuda')
    red.load_state_dict(model["model_state_dict"])
    print("Model loaded and sent to CUDA")
    classes = ['alzheimer-diagnosis','brain-tumor-clasification','tuberculosis-pneumonia-diagnosis'] 
